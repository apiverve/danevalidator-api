...
class DanevalidatorValidationException implements Exception {
  final List<String> errors;

  DanevalidatorValidationException(this.errors);

  @override
  String toString() => 'DanevalidatorValidationException: ${errors.join("; ")}';
}
...
/// DANE Record Validator API client.
///
/// For more information, visit: https://apiverve.com/marketplace/danevalidator?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [record] (required) - The DANE/TLSA record string to validate
class DanevalidatorClient {
  final String apiKey;
  ...
  DanevalidatorClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/danevalidator',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [DanevalidatorValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw DanevalidatorValidationException(errors);
    }
  }

  /// Execute a request to the DANE Record Validator API.
  ///
  /// Parameters are validated before sending the request.
  Future<DanevalidatorResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw DanevalidatorException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return DanevalidatorResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw DanevalidatorException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw DanevalidatorException('Resource not found');
      } else {
        throw DanevalidatorException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is DanevalidatorException) rethrow;
      throw DanevalidatorException('Request failed: $e');
    }
  }
...
/// Exception thrown by the DANE Record Validator API client.
class DanevalidatorException implements Exception {
  final String message;

  DanevalidatorException(this.message);

  @override
  String toString() => 'DanevalidatorException: $message';
}
